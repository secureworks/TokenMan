...
class Organizations(ModuleBase):
    """Organizations retrieval class"""
    ...
    def _fetch_organizations(
        ...
        """Retrieve the Organizations for the given user
    ...
    def fetch(cls, state: RunState):
        ...
        # Fetch organizations
        organizations = cls._fetch_organizations(
            cls,
            access_token=state.token_cache.access_token,
            proxies=state.proxies,
        )
        organizations_count = len(organizations["value"])
        logging.info(f"\tOrganizations: {organizations_count}")

        # If organizations found, write to output file
        if organizations_count > 0:
            utc_now = datetime.now(timezone.utc).strftime("%Y%m%d%H%M%S")
            filename = state.output / f"fetch.organizations.{utc_now}.json"
