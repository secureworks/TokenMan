...
class SharePoint(ModuleBase):
    ...
    def _search(
        ...
        """Search SharePoint files via Graph API accessible by the given user
    ...
    def search(
        ...
        """Search SharePoint files via Graph API

        :param state: run state
        :param search: search term
        """
        # Check if we have an access token for SharePoint
        if not cls.check_token(
            ...
            logging.debug(f"Acquiring new token for: 'SharePoint'")

            # Acquire a new token specific for SharePoint
        ...
        logging.info(f"Searching 'SharePoint' for: {keywords}")
        ...
        if total_results > 0:
            ...
            filename = state.output / f"search.sharepoint.{utc_now}.json"
