...
class OneDrive(ModuleBase):
    ...
    def _search(
        ...
        """Search OneDrive files via Graph API accessible by the given user
        ...
        return self.msgraph_fetch(
            self,
            path=f"me/drive/search(q='{search}')",
    ...
    def search(
        ...
        """Search OneDrive files via Graph API
        ...
        logging.info(f"Searching 'OneDrive' for: {keywords}")
        ...
        if total_results > 0:
            ...
            filename = state.output / f"search.onedrive.{utc_now}.json"
